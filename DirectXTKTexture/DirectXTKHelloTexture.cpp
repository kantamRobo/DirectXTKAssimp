...
#include "DirectXTKHelloTexture.h"
...
DirectXTKHelloTexture::DirectXTKHelloTexture(DX::DeviceResources* DR, UINT width, UINT height, std::wstring name)
{
...
void DirectXTKHelloTexture::Draw(DX::DeviceResources* DR) {
...
HRESULT DirectXTKHelloTexture::CreateShaders(const DX::DeviceResources* deviceResources)
{
    //パイプラインステートの作成
...
HRESULT DirectXTKHelloTexture::CreateBuffers(DX::DeviceResources* DR, int width, int height)
{
...
HRESULT DirectXTKHelloTexture::CreateTexture(DX::DeviceResources* DR, const wchar_t* path)
{
    auto device = DR->GetD3DDevice();

    HRESULT hr = DirectX::CreateWICTextureFromFile(device, path,
